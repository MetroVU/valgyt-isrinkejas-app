...
export default function ResultsView({ session, onPickRandom, onSpinWheel, onPickFromMatches, onReset, onChooseSuggestion }: ResultsViewProps) {
  ...
  const suggestions = suggestAlternatives(p1, p2, 3);
            ...
            <div className="bg-red-500/20 rounded-2xl p-8 border border-red-500/30">
              ...
              {suggestions.length > 0 && (
                <div className="mt-6 text-left">
                  <h3 className="text-lg font-bold mb-3">Pasiūlymai pagal jūsų skonius:</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {suggestions.map(s => (
                      <div key={s.id} className="bg-gray-800/60 rounded-xl p-3 flex items-center gap-3">
                        <span className="text-2xl">{s.image}</span>
                        <div className="flex-1">
                          <div className="font-semibold">{s.name}</div>
                          <div className="text-xs text-gray-400">{s.cuisine} • {s.platform === 'bolt' ? 'Bolt' : s.platform === 'wolt' ? 'Wolt' : 'Kita'}</div>
                        </div>
                        {onChooseSuggestion && (
                          <button
                            onClick={() => onChooseSuggestion(s.id)}
                            className="px-3 py-2 rounded-lg bg-gradient-to-r from-green-600 to-emerald-600 text-white text-xs hover:scale-105"
                          >
                            Pasirinkti
                          </button>
                        )}
